(function (lettuce) {
	...
	function showLove() {
		...
		v = 32; M = Math; R = M.random; C = M.cos; Y = 6.3;
		for (i = 0; i < Y; i += 0.2)
		...
		for (i = 0; i < v;)
		{
		    ...
		    f = [];
		    for (k = 0; k < v;)
		        f[k++] = { x: x, y: y, X: 0, Y: 0, R: 1 - k / v + 1, S: R() + 1, q: ~~(R() * v), D: 2 * (i % 2) - 1, F: 0.2 * R() + 0.7, f: "hsla(" + ~~H + "," + ~~S + "%," + ~~B + "%,.1)" };
            e[i++] = f
		}
	...
	show(undefined, 4000).then(
		...
		function() {
			return show(down, 4000)
		}
	).then(
		function() {
			return show(yours, 4000)
		}
	).then(
		function() {
			return show(together, 4000)
		}
	).then(
		function() {
